...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1BPtKekEv3?p=61)
...
随着互联网技术的快速发展，网络时代的到来，网络信息也将会改变当今社会。各行各业在日常企业经营管理等方面也在慢慢的向规范化和网络化趋势汇合。智能社区服务小程序的信息化程度体现在将互联网与信息技术应用于经营与管理，以现代化工具代替传统手工作业。无疑，使用网络信息化管理使信息管理更先进、更高效、更科学，信息交流更迅速。

对于之前智能社区服务小程序的管理，大部分都是使用传统的人工方式去管理，这样导致了管理效率低下、出错频率高。而且，时间一长的话，积累下来的数据信息不容易保存，对于查询、更新还有维护会带来不少问题。对于数据交接也存在很大的隐患。如果采用电子化的存储方式就会带来很大的改善，而且给用户的查询带来了很大便利，因此设计一个智能社区服务小程序刻不容缓，能够提高信息的管理水平。
## 1.3系统设计思想
...
# 3系统分析
## 3.1可行性分析
通过对本智能社区服务小程序实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本智能社区服务小程序采用SSM框架，JAVA作为开发语言，是基于WEB平台的B/S架构系统。
...
因此，智能社区服务小程序在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本智能社区服务小程序采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本智能社区服务小程序的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。
...
图4.14 住户信息实体属性图
### 4.3.2数据库设计表
智能社区服务小程序需要后台数据库，下面介绍数据库中的各个表的详细信息：
...
|baoxiuwupin|varchar(200)|否||报修物品|
|baoxiuwenti|longtext|是|NULL|报修问题|
|baoxiuriqi|datetime|是|NULL|报修日期|
...
|danyuanhao|varchar(200)|是|NULL|单元号|
|fangxing|varchar(200)|否||房型|
|tupian|varchar(200)|是|NULL|图片|
|huxing|varchar(200)|是|NULL|户型|
|mianji|varchar(200)|否||面积|
|jiage|int(11)|否||价格|
|xiangxidizhi|varchar(200)|是|NULL|详细地址|
|ruzhuzhuangtai|varchar(200)|是|NULL|入住状态|
表4. 4家政服务
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|fuwuxiangmu|varchar(200)|否||服务项目|
|tupian|varchar(200)|是|NULL|图片|
...
|fuwuxiangmu|varchar(200)|是|NULL|服务项目|
|xiaoshijiage|int(11)|否||小时价格|
|yuyueshizhang|int(11)|否||预约时长|
|zongjiage|int(11)|否||总价格|
...
|xingming|varchar(200)|是|NULL|姓名|
|wuyefei|float|否||物业费|
|shuidianfei|float|否||水电费|
|kuandaifei|float|否||宽带费|
|cheweifei|float|否||车位费|
|ranqifei|float|否||燃气费|
|zongfeiyong|float|否||总费用|
|jiaofeishijian|varchar(200)|是|NULL|缴费时间|
|ispay|varchar(200)|是|未支付|是否支付|
|userid|bigint(20)|是|NULL|用户id|
表4. 11小区地图
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|xiaoqumingcheng|varchar(200)|否||小区名称|
|jianzhumingcheng|varchar(200)|否||建筑名称|
|tupian|varchar(200)|是|NULL|图片|
|weizhi|varchar(200)|是|NULL|位置|
|jianshu|longtext|是|NULL|简述|
表4. 12用户
...
|louceng|varchar(200)|是|NULL|楼层|
|danyuanhao|varchar(200)|是|NULL|单元号|
|shifouruzhu|varchar(200)|是|NULL|是否入住|
|ruzhushijian|date|是|NULL|入住时间|
|huzhuxingming|varchar(200)|否||户主姓名|
|xingbie|varchar(200)|是|NULL|性别|
|nianling|varchar(200)|是|NULL|年龄|
|zhiye|varchar(200)|是|NULL|职业|
|gongzuodanwei|varchar(200)|是|NULL|工作单位|
|jiatingrenshu|varchar(200)|否||家庭人数|
